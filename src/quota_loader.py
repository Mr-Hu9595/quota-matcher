...
class QuotaLoader:
    ...
    # 定额编号格式正则（按专业）
    # 注：允许第三段含字母后缀（如 HA1、HA2、B1 等），否则会遗漏部分定额
    CODE_PATTERNS = {
        "河南省安装工程": r'^\d+-\d+-[A-Za-z0-9]+$',   # X-X-X (如 4-9-165, 1-3-HA1)
        "河南省市政工程": r'^\d+-\d+-[A-Za-z0-9]+$',   # X-X-X (如 5-3-385)
        "河南省房屋建筑与装饰工程": r'^\d+-[A-Za-z0-9]+$',     # X-XX (如 1-12, 1-3-HA1)
        "河南省城市轨道交通工程": r'^\d+-\d{3}$',    # X-XXX (如 1-001)
        "河南省城市地下综合管廊工程": r'^\d+-\d+-[A-Za-z0-9]+$',  # X-X-X
        "河南省绿色建筑工程": r'^\d+-[A-Za-z0-9]+$',           # X-XX
        "河南省装配式建筑工程": r'^\d+-[A-Za-z0-9]+$',  # X-字母数字 (如 1-HA1)
        "河南省市政公用设施养护维修": r'^\d+-[A-Za-z0-9]+$',   # X-XX
    }

    # 默认编号格式（兼容旧格式）
    DEFAULT_CODE_PATTERN = r'^\d+-\d+(\-[A-Za-z0-9]+)?$'
